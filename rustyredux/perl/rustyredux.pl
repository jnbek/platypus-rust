...
sub debug_lib { "$FindBin::Bin/../target/debug/librustyredux.so" }
...
sub main {
    ...
    $ffi->attach( make_point   => [ 'int',    'int' ]    => 'opaque' );
    $ffi->attach( get_distance => [ 'opaque', 'opaque' ] => 'double' );
    #print Dumper($ffi);
    my $p1 = make_point( 2, 2 );
    my $p2 = make_point( 4, 4 );
    my $result = get_distance($p1,$p2);
